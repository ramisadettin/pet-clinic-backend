...
const getAdoptionAds = async (req, res) => {
  ...
  try {
    ...
    const conn2 = await createConnection(connData)
    const [breeds] = await conn2.execute(`SELECT name FROM breeds WHERE type_name = ? ${req.query.post_type ? '' : ' OR 1=1'}`, [req.query.post_type ? req.query.post_type : ''])
    await conn2.end()

    const conn = await createConnection(connData, {debug: true})
    ...
    AND (ad.ad_type=? ${req.query.post_type ? '': ' OR 1=1'})
    ...
    const [adoptionAds] = await conn.execute(sqlQuery, 
      ...
      req.query.post_type ? req.query.post_type : '',
